...
const MyBookings = () => {
  ...
  const [deletedBookingData, setDeletedBookingData] = useState(null);

  // Handle Modal
  const modalHandler = () => {
    // Delete the selected booking information from database
    deleteBooking(deletedBookingData._id)
      .then((data) => {
        if (data.status === "fail") {
          ...
          updateRoomBookedStatus(deletedBookingData.roomId, false).then(
            (data) => {
              if (data.status === "fail") {
                toast.error(data.message);
              }
            ...
            <table className="min-w-full leading-normal">
              ...
              <tbody>
                {bookings.map((booking) => (
                  <TableRow
                    setDeletedBookingData={setDeletedBookingData}
                    booking={booking}
                    key={booking._id}
                    openModal={openModal}
                  />
